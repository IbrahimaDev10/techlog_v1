    ...
    if (in_array($fileActualExt, $allowed)) {
        if ($fileError === 0) {
            if ($fileSize < 1000000000) { // taille maximale de 1 Mo
                ...
                $req =$bdd->prepare("INSERT INTO fichier_reception_avaries (nom_fichier_recep_av, path_fichier_recep_av, taille_fichier_recep_av, type_fichier_recep_av, id_fichier_recep_av) VALUES (?,?,?,?,?)");
...
$fichier=$bdd->prepare('select * from fichier_reception_avaries where id_fichier_recep_av=?');
...
<div class="container-fluid"  id="recap_bl"  style="width: 50%; background: white;" >
  <div class="row">
   ...
   <?php $id=$_GET['id'];
   $donnees=ticket_recep_avaries($bdd,$id);
  ...
  <div style="float: right; margin-top: 15px;" class="col col-md-12  col-lg-12"> <h6 class="segment"> BL N°:  <?php echo $dnn['bl_ra']; ?> </h6></div><br>
...
<br><br>
        ...
        <?php if($dnn['sac_flasque_ra']!=0){?>
        <div style="float: left; margin-top:15px;" class="col col-md-6 col-lg-6"> <h6 class="segment"> NOMBRE SAC FLASQUE : <span class="titre_info"> <?php echo $dnn['sac_flasque_ra']; ?> </span></h6></div>
         <div style="margin-top:15px;" class="col col-md-6 col-lg-6"> <h6 class="segment"> POIDS FLASQUE <span class="titre_info"> <?php echo $dnn['poids_flasque_ra']; ?> </span></h6></div>
          <?php   } ?>
            <?php if($dnn['sac_mouille_ra']!=0){?>
        <div style="float: left; margin-top:15px;" class="col col-md-6 col-lg-6"> <h6 class="segment"> NOMBRE SAC MOUILLE : <span class="titre_info"> <?php echo $dnn['sac_mouille_ra']; ?> </span></h6></div>
         <div style="margin-top:15px;" class="col col-md-6 col-lg-6"> <h6 class="segment"> POIDS MOUILLE <span class="titre_info"> <?php echo $dnn['poids_mouille_ra']; ?> </span></h6></div>
          <?php   } ?>
      

       
       <div style="float:left;" class="col col-md-6 col-lg-4"> <h6 class="segment"> CAMIONS: <span class="titre_info"> <?php echo $dnn['camion_ra']; ?> </span></h6></div>
       <div  class="col col-md-4 col-lg-4"> <h6 class="segment"> CHAUFFEUR: <span class="titre_info"> <?php echo $dnn['chauffeur_ra']; ?> </span></h6></div>
    ...
    <div style="float: left;" class="col col-md-4 col-lg-4">
       ...
       <div style="float: right;" class="col-md-4 col-lg-4">
        ...
        <?php while($fiche=$fichier->fetch()){
          ...
          ?>
          <div id="<?php echo $fiche['id_fich_recep_av'].'entrepot' ?>">
          <a target="blank_page" href="<?php echo $fiche['path_fichier_recep_av']; ?>" style="float: left;"><i class="fas fa-file-pdf " style="color: red; "></i><?php echo $fiche['nom_fichier_recep_av'] ?></a><a style="float:right;"  id="<?php echo $fiche['id_fichier_recep_av'] ?>" name="deleteMg"   class="fabtn1 " onclick="deleteFichier(<?php echo $fiche['id_fichier_recep_av'] ?>)" > <i class="fa fa-trash   " ></i> </a> <br><br>
          </div>
        <?php } ?>
