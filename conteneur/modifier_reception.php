...
if(!empty($_POST['dates'])  and !empty($_POST['sain'])  and !empty($_POST['id_connaissement']) and !empty($_POST['id_num_conteneur'])  and !empty($_POST['camion']) and !empty($_POST['id_transporteur'])){
	$dates=$_POST['dates'];
	$sain=$_POST['sain'];
	...
	$insertion=$bdd->prepare("UPDATE reception_conteneur SET dates=?,sain=?,flasque=?,mouille=?,poids_rc=?,id_num_conteneur=?,camion=?,id_transporteur=? WHERE id_recep=?");
	...
	$insertion->bindParam(7,$camion);
	$insertion->bindParam(8,$id_transporteur);
	$insertion->bindParam(9,$id);
	$insertion->execute();
