<?php 
 ...
 function compterecond($bdd,$produit,$poids_sac,$navire,$destination){
     $compterecond=$bdd->prepare("SELECT dis.id_produit,dis.poids_kg,nc.id_navire,mg.id,  count(recon.declaration_id),dt.id_declaration_transfert from reconditionnement_reception_transfert as recon 
      inner join declaration_transfert as dt on   dt.id_declaration_transfert=recon.declaration_id
    ...
    inner join mangasin as mg on mg.id=recon.id_destination
     ...
     WHERE dis.id_produit=? and dis.poids_kg=? and nc.id_navire=? and recon.id_destination=?");
 ...
 $compterecond->execute();
  ...
  function recond($bdd,$produit,$poids_sac,$navire,$destination){
     $recond=$bdd->prepare("SELECT dis.id_produit,dis.poids_kg,dis.id_mangasin,nc.id_navire,  recon.declaration_id,dt.id_declaration_transfert,recon.*,mg.id from reconditionnement_reception_transfert as recon 
      inner join declaration_transfert as dt on dt.id_declaration_transfert=recon.declaration_id
