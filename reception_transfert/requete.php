...
//-------------------------------requetes PV DE RECONDITIONNEMENT
    ...
    $recondPV = $bdd->prepare("SELECT count(sac_av_recond), sum(sac_eventres), sum(sac_av_recond), sum(poids_av_recond),sum(sac_balayure_recond), sum(poids_balayure_recond)  from reconditionnement_reception
                   WHERE id_dis_recond=? ");
     ...
     $SainsPV = $bdd->prepare("SELECT poids_sac_recep,  sum(sac_recep), sum(poids_recep)  from reception
        ...
        $recondPV ->execute();
                  ...
                  $SomAvrPV = $bdd->prepare("SELECT  sum(sac_flasque_avr),sum(sac_mouille_avr) from avaries_de_reception
                   WHERE id_dis_avr=? ");
