...
if(isset($_POST['valider_banque'])){
	if(!empty($_POST['banque'])){
		...
		$tel=$_POST['telephone'];
		$email=$_POST['email'];
     $verify = $bdd->prepare("select banque from banque where banque=?");
$verify->bindParam(1,$banque);
$verify->execute();
$row=$verify->fetch();

if(!$row){
$insertbanque = $bdd->prepare("INSERT INTO banque(banque,adresse_banque,tel_banque,email_banque) VALUES(?,?,?,?)");
$insertbanque->bindParam(1,$banque);
$insertbanque->bindParam(2,$adresse);
$insertbanque->bindParam(3,$tel);
$insertbanque->bindParam(4,$email);

$insertbanque->execute();
...
$Lesbanques=$bdd->query("select * from banque ");
	...
	<div class="content-start transition" style="background-image: url('../images/bg_page.jpg'); background-repeat:no-repeat; background-size: 100%;  margin: 0px; border: none; border-radius: 0px;">
		<div class="container-fluid dashboard">
			...
			<div class="container-fluid" >
				<div class="row">
					<center>
			<h3 class="ajout_dis" style="color: white;">INSERTION banque</h3>
			</center>
  ...
  <div class="modal-dialog">
     <div class="modal-content" style=" border: solid; border-color:rgb(0,141,202); ">
            <div class="modal-header" style="">
              <center>
              	<h3 class="modal-title fs-10 text-white text-center" id="exampleModalLabel " style="text-align:center;   ">AJOUT banque</h3></center>
              	<center>
...
<div class="form-group position-relative has-icon-left mb-5">
  ...
  <input type="text" class="form-control"  id="exampleFormControlInput1" placeholder="banque" name="banque"><br>
  ...
  <input type="text"  class="form-control"  id="telephoneInput" placeholder="TELEPHONE" name="telephone" oninput="formatTelephone()" ><br>
   <input type="text" class="form-control"  id="exampleFormControlInput1" placeholder="EMAIL" name="email"><br>
         ...
         <center>
        <button type="submit" class="btn btn-primary btn-block btn-lg shadow-lg mt-5" name="valider_banque">valider</button>
					</form>
 ...
 <?php 
      ...
      $cal=$calculLigne->fetch();              	
                          ...
                          <tr style="text-align:center;" border='5' id="<?php echo $row['id'] ?>">
                          	<td id="coltd" ><span style="color: red; margin-left: 0px; " >	<?php echo  $cal['count(banque)']; ?></span> </td>
                          	<td id="coltd">	<?php echo  $row['banque']; ?> </td>
                           </td>
                          	<td id="coltd">	<?php echo  $row['adresse_banque']; ?> </td>
                          	<td id="coltd">	<?php echo  $row['tel_banque']; ?> </td>
                          	
                          	<td id="coltd">	<?php echo  $row['email_banque']; ?> </td>

                             <td id="coltd">
                          	<button  id="<?php echo $row['id'] ?>" name="deletecli" type="submit"  class="fabtn1 " onclick="deletebanque(<?php echo $row['id'] ?>)" > <i class="fa fa-trash  " ></i> </button>
                          	<a class="fabtn" type="" name="modify"  href="modifier_banque.php?id=<?php echo $row['id']; ?>"       id="btnbtn" > <i class="fa fa-edit " ></i></a></td>         
                              </tr>
                      <?php } ?>	
