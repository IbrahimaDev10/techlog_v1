...
$afficher=$bdd->prepare("SELECT dis.*, cli.client,nc.num_connaissement,nc.poids_kg, nc.poids_connaissement,nc.id_navire, mg.mangasin, p.produit,p.qualite,q.* from dispats as dis
...
<?php $choix_client=$bdd->prepare('SELECT nc.num_connaissement,nc.id_navire,cli.client,cli.id   from client as cli
	...
	<select name="produit" id='produit' <?php if($navs['type']=='VRAQUIER'){ ?> style="max-width: 30%; display: none;" <?php } ?> <?php if($navs['type']=='SACHERIE'){ ?> style="max-width: 30%; " <?php } ?> data-role='detail_produit'>
	...
	<div id="poids_sac_vrac" style="display: none;">
                           	...
                           	<?php $choix_qualite=$bdd->query('SELECT * from qualite_produit_vrac ');
	   ?>

                           	<select name="" id='qualite' style="max-width: 50%;" >
		<option>CHOISIR QUALITE</option>
		<?php while($qual=$choix_qualite->fetch()){ ?>
			<option value="<?php echo $qual['id_qual']; ?>" ><?php echo $qual['qualite_vrac']; ?> </option>
		<?php } ?>
...
<tr  style="background: white; color:black; text-align:center; vertical-align: middle;">
	...
	<td><?php echo $aff['produit'] ?>  <?php if($navs['type']=='SACHERIE'){ echo $aff['qualite'];  ?> <?php echo $aff['poids_kg'].' KG'; } ?> <?php if($navs['type']=='VRAQUIER'){ echo $aff['qualite_vrac']  ?> <?php if($aff['poids_kgs']!=0){ echo $aff['poids_kg'].' KG'; } } ?> </td>
...
$(document).on('click', 'a[data-role=ajout_dispat]', function() {
    ...
    if (connaissement != '' && poids != '' && produit !='' && destination != '' && destination_douaniere !='' && id_navire != '' && type === 'SACHERIE') {
        ...
        if (connaissement != '' && poids != ''  && destination != '' && destination_douaniere != '' && id_navire != '' && type_chargement!='' && poids_sac_en_vrac!='' && qualite!='' && type === 'VRAQUIER') {
        formData.append('connaissement', connaissement);
        formData.append('destination', destination);
        formData.append('poids', poids);
        formData.append('id_navire', id_navire);
