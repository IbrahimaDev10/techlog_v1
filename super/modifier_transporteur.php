...
$res4n= $bdd->prepare("select nom from transporteur where id=?  ");
...
if (isset($_POST['modifier_transporteur'])) {
...
try{
     ...
     $insertRecep1= $bdd->prepare("UPDATE transporteur set nom=?  where id=? "); 



$insertRecep1->bindParam(1,$nom); 
...
echo '<script>window.location.href = "gestion_donnees.php"</script>';
...
<center>
  ...
  <input type="text" class="form-control"  id="exampleFormControlInput1" placeholder="chauffeur" name="nom" value="<?php echo $nom1 ?>" >
</div>
         ...
         <center>
        <button type="submit" class="btn btn-primary btn-block btn-lg shadow-lg mt-5" name="modifier_transporteur">valider</button>
</form> 
      </div>   
