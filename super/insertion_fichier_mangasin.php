...
$titremangasin= $bdd->prepare("SELECT id_dis FROM dispat

                   WHERE id_dis=?  ");
        $titremangasin->bindParam(1,$_GET['id']);
       $titremangasin->execute();

$resid= $bdd->prepare("SELECT id_dis FROM dispat
...
$fichier=$bdd->prepare('select * from fichier_mangasin where id_fichier_dis=?');
...
<?php } ?>
 ...
 <div class="mb-3">
        ...
        <button style="float: left;" type="submit" class="btn btn-primary " style="text-align: center;" name="fichier_mg" id="deb"  >Joindre</button>
        <br><br>
</form> 
      </div> 
      <div style="float: right;"></div> 
       ...
       <div class="mb-3">
        <?php while($fiche=$fichier->fetch()){
          ...
          ?>
          <div id="<?php echo $fiche['id_fich_mg'] ?>">
          <a target="blank_page" href="<?php echo $fiche['path_fichier_mg']; ?>" style="float: left;"><i class="fas fa-file-pdf " style="color: red; "></i><?php echo $fiche['nom_fichier_mg'] ?></a><a style="float:right;"  id="<?php echo $fiche['id_fichier_dis'] ?>" name="deleteMg"   class="fabtn1 " onclick="deleteFichier(<?php echo $fiche['id_fich_mg'] ?>)" > <i class="fa fa-trash   " ></i> </a> <br><br>
          </div>
        <?php } ?>
  ...
  function deleteFichier(id){
       ...
       if(confirm('Voulez vous vraiment supprimer ce fichier?')){
         ...
         $.ajax({
              ...
              type:'post',
              url:'delete_fichier_mangasin.php',
              data:{delete_id:id},
