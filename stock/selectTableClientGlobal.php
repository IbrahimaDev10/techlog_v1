...
if(isset($_POST["idClientGlobal"])){
    
    $clients=$bdd->prepare("select * from client order by client asc");
    $clients->execute();
...
<html lang="fr">
    ...
    <?php while ($row=$clients->fetch()) {?> 
        <table class='table table-hover table-bordered table-striped' id='tableStock' border='2' >
            ...
            WHERE dis.id_client =?
            GROUP BY dis.id_navire,dis.id_mangasin,dis.id_produit,dis.id_dis
            with rollup");
            ...
            while ($rowa=$stock1->fetch()) {
                ...
                if(!empty($idNavire) and !empty($idDestination) 
               ...
               //Livraison
                ...
                WHERE id_client_recond_liv=? and id_destination_recond_liv=? 
                and id_navire_recond_liv =? and id_produit_recond_liv=?
                and poids_sac_recond_liv=?
                GROUP BY id_dis_recond_liv");
                $stock7->bindParam(1,$idClient);
                $stock7->bindParam(2,$idDestination);
                $stock7->bindParam(3,$idNavire);
                $stock7->bindParam(4,$idProduit);
                ...
                <?php
                 ...
                 if(!empty($idNavire) and !empty($idDestination) and 
                ...
                <tr class="trGestionStockTotal">
                    <td colspan="3">TOTAL <?php echo "<br>". $row['client'];?> </td>
