...
if(isset($_POST["idNavireGlobal"])){
    
    $navires=$bdd->prepare("select * from navire_deb order by navire asc");
    $navires->execute();
...
<html lang="fr">
    ...
    <body >
    <?php while ($row=$navires->fetch()) {?> 
        <table class='table table-hover table-bordered table-striped' id='tableStock' border='2' >
            <thead >
                <tr class="titreStock">
                    <td colspan="15">
                    <?php $navireSelect = $row['navire'];
                    echo $navireSelect; ?> 
                    </td>
                </tr>
                <tr class="trGestionStock" >
                    <td scope="col"  rowspan="3" class="rowGestionStock">ENTREPOTS</td> 
            ...
            WHERE id_navire =?
            GROUP BY dis.id_mangasin,dis.id_client,dis.id_produit,dis.id_dis
            with rollup");
            ...
            while ($rowa=$stock1->fetch()) {
                ...
                if(!empty($idDestination) and !empty($idClient) 
                ...
                $PoidsRecep = ($sommeSacRecep*$sacRecepPoids)/1000;
                ...
                $sommePoidsRecondRec = ($sommeSacsRecondRec*$sacRecepPoids)/1000;
                ...
                WHERE id_navire_recond_liv =? and id_destination_recond_liv=? and id_produit_recond_liv=?
                and id_client_recond_liv=? and poids_sac_recond_liv=?
                GROUP BY id_dis_recond_liv");
                $stock7->bindParam(1,$idNavire);
                $stock7->bindParam(2,$idDestination);
                ...
                <?php
                 ...
                 if(!empty($idDestination) and !empty($idClient) and 
                 empty($idProduit) and empty($idDis)){
                    ...
                    ?>
                    <tr class="tr_data_gestion_stock" >
                        <td colspan="3">TOTAL <?php echo " ".$rowa['mangasin'] ;?></td>
                ...
                <tr class="trGestionStockTotal">
                    <td colspan="3">TOTAL <?php echo "<br>". $row['navire'];?> </td>
