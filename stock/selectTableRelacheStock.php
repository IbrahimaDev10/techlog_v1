...
if(isset($_POST["idRelache"])){
    $idRelache=$_POST["idRelache"];
    $relaches=$bdd->prepare("select * from client where id=?");
    $relaches->bindParam(1,$idRelache);
    $relaches->execute();
    ...
    WHERE dis.id_client =?
    GROUP BY dis.id_navire,dis.id_dis 
    with rollup");
    $stock1->bindParam(1,$idRelache);
    $stock1->execute();
    ...
    <body >
        <table class='table table-hover table-bordered table-striped' id='tableStock' border='2' >
            <thead >
                <tr class="titreStock">
                    <td colspan="13">
                    <?php while ($row=$relaches->fetch()) {
                            $relacheSelect = $row['client'];
                        }
                        echo $relacheSelect; ?> 
                    </td>
                </tr>
            ...
            <tbody> 
                ...
                if(!empty($idNavire) and !empty($idDis)) {     
               ...
               <?php
                ...
                <tr class="trGestionStockTotal">
                    <td colspan="3">TOTAL <?php while ($row=$relaches->fetch()) {$relacheSelect = $row['cient'];}echo $relacheSelect; ?> </td>
