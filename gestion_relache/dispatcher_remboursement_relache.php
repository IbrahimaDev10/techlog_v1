...
$afficher=$bdd->prepare("select tr.*, dis.* from dispatching as dis 
...
<?php 
        ?>
               <select id="relache_dispatcher" data-role="choix_relache_dispatcher">
               	<option>choisir un connaissement à rembourser</option>
                ...
                <select id="destination">
               	<option>choisir une destination</option>
                            ...
                            <input id="quantite" type="text" class="" placeholder="quantite" name="manifeste" style="width:45%; margin-right:20px ">

                                   
				
				
   <center>
<a class="btn" style="width: 100%; " data-role="inserer_dispath_relache">Ajouter</a>
   </center> 
  ...
  //$('#type').css('display', 'block');
    ...
    var id_navire = $('#navire').val();
      //var type_dec = $('#type_dec').val();
        ...
        $.ajax({
        url:'afficher_select_rd.php',
        method:'post',
        data:{id_navire:id_navire},
        success: function(response){
            $('#relache_dispatcher').html(response);
...
<script type="text/javascript">
  $(document).ready(function(){
    $(document).on('click','a[data-role=inserer_dispath_relache]',function(){
  //$('#type').css('display', 'block');
    ...
    var quantite = $('#quantite').val();
      ...
      //var type_dec = $('#type_dec').val();
        ...
        $.ajax({
        url:'insertion_dispatching_relache.php',
        method:'post',
        data:{connaissement:connaissement,navire:navire,destination:destination,quantite:quantite},
        success: function(response){
