...
$rob_dec_verif_dec=$bdd->prepare("SELECT ex.poids_declarer_extends, re.id_trans_reelle, sum(rm.sac), sum(rm.poids),dis.id_dis,dc.*,nc.* from transit_extends as ex left join register_manifeste as rm on ex.id_trans_extends=rm.id_declaration
  ...
  inner join declaration as dc on dc.id_declaration=ex.id_declaration_extends          
        WHERE dis.id_produit=? and  dis.poids_kg=? and nc.id_navire=? and dis.id_mangasin=? and ex.id_trans_extends=?   ");
...
function control_excedent_sur_declaration2($bdd,$produit,$poids_sac,$navire,$destination,$declaration){
          $rob_dec_verif_dec2=$bdd->prepare("SELECT ex.poids_declarer_extends, re.id_trans_reelle, sum(tr.poids_flasque_tr_av), sum(tr.poids_mouille_tr_av),dis.id_dis,dc.*,nc.* from transit_extends as ex left join transfert_avaries as tr on ex.id_trans_extends=tr.id_declaration_tr
  inner join transit_reelle as re on re.id_trans_reelle=ex.id_trans_reelle
  inner join dispat as dis on dis.id_dis=ex.id_bl_extends
   inner join numero_connaissement as nc on nc.id_connaissement=dis.id_con_dis
  inner join declaration as dc on dc.id_declaration=ex.id_declaration_extends 
