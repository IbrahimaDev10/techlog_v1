...
if(isset($_POST['id'])){
   ...
   $d1=str_replace(' ','', $date);
	 ...
	 $poids=$_POST['poids'];
	$id=$_POST['id'];
	$update=$bdd->prepare("UPDATE register_manifeste set dates=?, heure=?, bl=?, camions=?, chauffeur=?, id_declaration=?, cale=?,  poids=?  where id_register_manif=?");
...
<?php }
if($diff>=0){ ?>
  <td  style="color: green;"><?php echo "RAS" ?> <?php if($en_cours=$cherche_en_cours->fetch()){  ?><br> En cours <?php } ?>
