...
$titremangasin= $bdd->prepare("SELECT id_tr_avaries FROM transfert_avaries

                   WHERE id_tr_avaries=?  ");
        $titremangasin->bindParam(1,$_GET['id']);
       $titremangasin->execute();

$resid= $bdd->prepare("SELECT id_tr_avaries FROM transfert_avaries
    ...
    if (in_array($fileActualExt, $allowed)) {
        if ($fileError === 0) {
            if ($fileSize < 1000000000) { // taille maximale de 1 Mo
                ...
                $req =$bdd->prepare("INSERT INTO fichier_transfert (nom_fichier_trans, path_fichier_trans, taille_fichier_trans, type_fichier_trans, id_fichier_transfert) VALUES (?,?,?,?,?)");
...
$fichier=$bdd->prepare('select * from fichier_transfert where id_fichier_transfert=?');
...
<div class="container-fluid"  id="recap_bl"  style="width: 50%; background: white;" >
  <div class="row">
   ...
   <?php $id=$_GET['id'];
   $variable=afficher_variable($bdd,$id);
   if($var=$variable->fetch()){
    $produit=$var['id_produit'];
    $poids_sac=$var['poids_kg'];
    $navire=$var['id_navire'];
    $destination=$var['id_mangasin'];
   echo '<span style="color:red;">' .$produit. '</span>';

   
   $donnees=segment($bdd,$produit,$poids_sac,$navire,$destination);
   echo '<span style="color:red;">' .$produit. '</span>';
  ...
  <div style="float: right;" class="col col-md-12  col-lg-12"> <h6 class="segment"> BL N°:  <?php echo $dnn['bl_tr']; ?> </h6></div><br>
      ...
      <div style="float: left;" class="col col-md-4 col-lg-4"> <h6 class="segment"> NAVIRE: <span  class="titre_info"> <?php echo $dnn['navire']; ?> </span></h6></div>
       ...
       <div   class="col col-md-4 col-lg-4"> <h6 class="segment"> CONNAISSEMENT: <span class="titre_info"> <?php echo $dnn['num_connaissement']; ?> </span></h6></div>
        <div   class="col col-md-6 col-lg-6"> <h6 class="segment"> DECLARTION/TRANSFERT: <span class="titre_info"> <?php echo $dnn['num_declaration']; ?> </span></h6></div>
        ...
        <?php if($dnn['sac_flasque_tr_av']!=0){ ?>
        <div style="float: left; margin-top:5px;" class="col col-md-6 col-lg-6"> <h6 class="segment"> NOMBRE SAC FLASQUE: <span class="titre_info"> <?php echo $dnn['sac_flasque_tr_av']; ?> </span></h6></div>
         <div style="margin-top:5px;" class="col col-md-6 col-lg-6"> <h6 class="segment"> POIDS FLASQUE: <span class="titre_info"> <?php echo $dnn['poids_flasque_tr_av']; ?> </span></h6></div>
       <?php } ?>

        <?php if($dnn['sac_mouille_tr_av']!=0){ ?>
        <div style="float: left;" class="col col-md-6 col-lg-6"> <h6 class="segment"> NOMBRE SAC MOUILLE: <span class="titre_info"> <?php echo $dnn['sac_mouille_tr_av']; ?> </span></h6></div>
         <div  class="col col-md-6 col-lg-6"> <h6 class="segment"> POIDS MOUILLE: <span class="titre_info"> <?php echo $dnn['poids_mouille_tr_av']; ?> </span></h6></div>
       <?php } ?>
      ...
      <div style="float: right;"></div> 
       ...
       <div class="mb-3">
        <?php while($fiche=$fichier->fetch()){
          ...
          ?>
          <div id="<?php echo $fiche['id_fich_trans'].'entrepot' ?>">
          <a target="blank_page" href="<?php echo $fiche['path_fichier_trans']; ?>" style="float: left;"><i class="fas fa-file-pdf " style="color: red; "></i><?php echo $fiche['nom_fichier_trans'] ?></a><a style="float:right;"  id="<?php echo $fiche['id_fich_trans'] ?>" name="deleteMg"   class="fabtn1 " onclick="deleteFichier(<?php echo $fiche['id_fich_trans'] ?>)" > <i class="fa fa-trash   " ></i> </a> <br><br>
          </div>
        <?php } ?>
  ...
  function deleteFichier(id){
       ...
       if(confirm('Voulez vous vraiment supprimer ce fichier?')){
         ...
         $.ajax({
              ...
              type:'post',
              url:'delete_fichier_transfert_avaries.php',
              data:{delete_id:id},
